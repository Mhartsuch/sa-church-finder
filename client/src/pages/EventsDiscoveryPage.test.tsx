...
describe('EventsDiscoveryPage', () => {
  ...
  describe('subscribe-to-calendar button', () => {
    ...
    it('labels the kid-friendly feed and serializes familyFriendly into the URL', () => {
      // Family-friendly-only narrowing reads naturally ("Subscribe to
      // kid-friendly events") and the feed URL carries `familyFriendly=true`
      ...
      const trigger = screen.getByRole('button', {
        name: /Subscribe to kid-friendly events/,
      });
      fireEvent.click(trigger);

      const apple = screen.getByRole('menuitem', { name: /Apple Calendar/ });
      expect(apple.getAttribute('href')).toMatch(/events\.ics\?familyFriendly=true$/);
    });

    it('uses the generic label and combines familyFriendly with another axis in the URL', () => {
      // Once a second axis is narrowed (family + accessible), the label
      // collapses to the generic wording and the URL chains every active
      // param so the saved filename surfaces the specifics.
      useEventsFeedMock.mockReturnValue({
        data: buildResponse([]),
        isLoading: false,
        isFetching: false,
      ...
      renderPage('/events?accessible=1&family=1');
