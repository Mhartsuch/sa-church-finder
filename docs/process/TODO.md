...
_Last updated: 2026-04-15 (events discovery feed now groups results under day headers — Today / Tomorrow / "Weekday, Month D" — via a new `client/src/lib/event-grouping.ts` helper with dedicated unit tests; same-day events collapse into a single section with an event count in the header, making busy feeds easier to scan; home page "Upcoming church events" module still previews the next week of events via `/api/v1/events` and deep-links into `/events`; events discovery feed supports a `timeOfDay` filter — `GET /api/v1/events?timeOfDay=morning|afternoon|evening` evaluated in San Antonio local time and a chip group on `/events`; `sachurchfinder.com` still needs a backend redeploy for the Prisma session-store auth fix)_
