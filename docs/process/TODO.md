...
_Last updated: 2026-04-15 (events discovery feed now supports multi-select event types — `GET /api/v1/events?type=service,community` OR-combines matching events, and the `/events` page exposes a chip-group selector instead of a single-value dropdown, with one removable active-filter chip per selected type; the wire format still accepts a single value, a comma-separated list, or repeated `type=` params and dedupes duplicates; the aggregated ICS subscribe button falls back to the city-wide feed when more than one type is selected, since a single calendar URL can only carry one type filter; day grouping, time-of-day, saved-church, keyword, and date-preset filters all continue to work alongside the new chips; `sachurchfinder.com` still needs a backend redeploy for the Prisma session-store auth fix)_
